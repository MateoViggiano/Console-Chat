...
struct InPacket {
    ...
    InPacket(const size_t senderID, ENetPacket*const packet, enet_uint8 channel=0) noexcept :senderID(senderID),packet(packet){}
    InPacket(InPacket&& other) noexcept :senderID(other.senderID), packet(other.packet){
        other.packet = nullptr;
    }
    InPacket& operator=(InPacket&& other) noexcept {
        senderID = other.senderID;
        packet = other.packet;
        other.packet = nullptr;
        return *this;
    }
    ENetPacket* drop() noexcept {
...
class Aplication {
        ...
        while (running) {
            while (server.host_service(0) > 0) { // Este bucle encola paquetes entrantes
                ...
                case ENET_EVENT_TYPE_DISCONNECT:
                    ...
                    file_relay_queue.push({ getID(peer),nullptr }); // same for the file relay thread
    ...
    void transfer_router_loop() {
        while (running) {
            ...
            while (file_relay_queue.try_pop(inpacket)) {
                if (inpacket->packet == nullptr) {
                    auto it = active_transfers.begin();
                    while (it != active_transfers.end()) {
                        if (it->val.x1 == inpacket->senderID || it->val.x2 == inpacket->senderID) { // Si el emisor o el receptor se desconectaron, la transferencia se termina
                            Info data;
                            size_t dest;
                            data.values.transferID = it->key;
                            if (it->val.x1 != inpacket->senderID) {
                                data.values.op = CANCEL_FILE_TRANSFER;
                                dest = it->val.x1;
                            }
                            else {
                                data.values.op = CANCEL_FILE_RECIEVE;
                                dest = it->val.x2;
                            }
                            outgoing_events.push({ {dest},enet_packet_create(&data,sizeof(data),ENET_PACKET_FLAG_RELIABLE),1 });
                            active_transfers.del(it++);
                        }
                        else ++it;
                    }
                }
                else try {
                    if (inpacket->packet->dataLength < sizeof(Info)) throw broken_packet();
                    switch (get_opcode(inpacket->packet->data)) {
                    case FILE_CHUNK: forward_file_chunk(*inpacket);break;
                    default: throw broken_packet();break;
                    }
                }
    ...
    void forward_file_chunk(InPacket& inpacket) {
        if (inpacket.packet->dataLength < sizeof(Info) + sizeof(bool)) throw broken_packet();
    ...
    void process_packet(InPacket inpacket) {
        try {
            ...
            case FILE_READY: send_file_ready(inpacket);break;
    ...
    void send_file_ready(InPacket& inpacket) {
        const Info& data = *reinterpret_cast<const Info*>(inpacket.packet->data);
        auto IDs = active_transfers.find(data.values.transferID);
        if (IDs == active_transfers.end()) throw transferID_not_found();
        if (inpacket.senderID != IDs->val.x2) throw senderID_mismatch();
        outgoing_events.push({ {IDs->val.x1},inpacket.drop(),0 });
    }
    ...
    void push_failure(size_t id, ClientOpcode op, enet_uint8 channel, const std::exception& e) {
        ...
        outgoing_events.push({ {id},enet_packet_create(data.get(),sizeof(ClientOpcode) + size,ENET_PACKET_FLAG_RELIABLE),channel});
    }
