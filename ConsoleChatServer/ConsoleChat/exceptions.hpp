...
class transferID_not_found : public std::exception {
public:
	const char* what() const noexcept override {
		return "Transfer ID not found";
	}
};
class senderID_mismatch : public std::exception {
public:
	const char* what() const noexcept override {
		return "Sender ID doesn't match with the one associated with the active transference";
	}
};
class userID_mismatch : public std::exception {
public:
	const char* what() const noexcept override {
		return "User ID doesn't match with the one associated with the file";
	}
};