...
private:
	...
	void file_loop() {
		while (context.running) {
			...
			while (context.incoming_files_queue.try_pop(opt)) {
				...
				if (ofile.stream.is_open()) context.incoming_files[opt->x1] = mpv::move(ofile);
			...
			while (context.outgoing_files_queue.try_pop(opt)) {
				...
				if (ifile.stream.is_open()) context.outgoing_files[opt->x1] = mpv::move(ifile);
			...
			while (context.incoming_file_chunks.try_pop(opt_packet)) {
				const Chunk& chunk = *reinterpret_cast<Chunk*>(opt_packet.value()->data);
				assert(chunk.op == FILE_CHUNK);
				size_t buffer_size = opt_packet.value()->dataLength - Chunk::info_size;
				auto it = context.incoming_files.find(chunk.transferID);
				it->val.stream.write(chunk.buffer, buffer_size);
				if (chunk.is_last_one) context.incoming_files.del(it);
			}
	...
	size_t process_transferenceID(const ENetPacket* packet) {
		...
		context.outgoing_files_queue.push({ transferID,*filename });
		return transferID;
	}
