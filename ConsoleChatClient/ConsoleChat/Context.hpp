...
struct oFile {
	...
	oFile(oFile&& other) : filename(std::move(other.filename)), stream(std::move(other.stream)) {}
...
struct iFile {
	...
	iFile(iFile&& other) : filename(std::move(other.filename)), stream(std::move(other.stream)), ready(other.ready) {}
...
public:
	...
	mpv::Map<size_t, mpv::String> not_ready_outgoing_file;
